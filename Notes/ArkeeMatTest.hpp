...
struct alignas(16) Vec4
{
	...
	*/
	...
	/*
	*	Why not const float& other as opposed to float other? A few reasons: Passin
	*  a reference is essentially analogous to passing a pointer and unpacking it. a pointer itself is
	*  8 bytes which is already more than afloat, so we barely/dont save time from the copy or memory AND 
	*  we have to do the extra step of unpacking it. (referencing vs dereferencing)
	* 
	* ALSO if you genuinely just pass a float,the float is directly put into a register for use so its
	* just faster.
	* 
	*  
	*/
	...
	{
		// Note that set1 stands for set all 4 floats of the packaged 128 bits from 1 input.
	...
	Vec4& operator/=(float other)
	{
		// _mm_div_ps is VERY slowly (3x as many cycles as mul)
		// and is only really a good tradeoff if ur diivng by anothervector
		// with different components

		// When dividing by a float,we can just use multiplication which is much faster!
